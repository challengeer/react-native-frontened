...
export default function InputBar({ onCancel, onSearch, label, description }: InputBarProps) {
    ...
    return (
        <View className="flex-row items-center px-4 mt-10">
            <View className="relative flex-1 flex-row items-center">
                ...
                <Text className="absolute top-14 text-sm text-neutral-500 dark:text-neutral-400">{description}</Text> {/* Maybe top-14 is not the best solution */}
            </View>
            {focused && (
                <Text
                    onPress={() => {
